...
async run(message,bot) {
  ...
  if (command.guilOwnerOnly) {
      ...
      ) return message.reply(" This command is only for guildOwner.");
	  }
  let neededPermissions = [];
	  ...
	  if(neededPermissions.length > 0){
		  return message.reply(` I don't have a ${neededPermissions.map((p) => `\`${p}\``).join(", ")} permissions.`)
	  }
	  neededPermissions = [];
	  ...
	  if(neededPermissions.length > 0){
		  return message.reply(` You don't have a ${neededPermissions.map((p) => `\`${p}\``).join(", ")} permissions`)
	 }
	...
	if (now < expirationTime) {
		...
		return message.reply(`please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`).then(msg=> msg.delete({ timeout:timeLeft.toFixed(1)*1000 }));
	}
	  }
